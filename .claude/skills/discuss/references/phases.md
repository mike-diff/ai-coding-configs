# /discuss — Phases and Subagent Briefs
...
```
You are the codebase scout for a /discuss session exploring: [IDEA]
...
Web researcher:
```
You are the web researcher for a /discuss session exploring: [IDEA]
...
```
You are the challenger for a /discuss session. Try to break this plan:
...
- **Medium/large or multi-phase** → recommend `/spec "[feature]"`; /spec inherits the
  ...
  inside /discuss — one spec generator in the toolbox, not two.
