---
name: dispatch
description: Unattended CI orchestrator. Runs the /dev team build headlessly for a GitHub issue labeled "dispatch", using native Task sub-agents. Use when invoked by the dispatch GitHub Actions workflow; not for interactive use. No human in the loop.
...
Run the `/dev` team build for a GitHub issue, fully unattended inside a GitHub Actions
...
<unattended>
The interactive `/dev` has two human-input STOPs. Both are SUSPENDED here:
- `/dev` Phase 3 "Clarify — STOP for user input" → there is no user. When the request is
...
- `/dev` Phase 6 Reflect "Questions for User" → becomes a note in the PR body, never a gate.
...
Run the `/dev` phases, skipping every human-input STOP. The authoritative phase-by-phase
...
replacement for `/dev`'s clarify STOP. Proceed without stopping.
</phase>

<phase name="build">
Spawn the **implementer** with the issue, the file map, and your Assumptions. Then run the
gate loop (max 5 iterations, per `/dev` Phase 5):
...
For risk-triggered work (auth, migrations, public API, new deps), use the `/dev` Phase 7
review council instead of the single reviewer.
</phase>

<phase name="reflect">
Self-review scope and Assumptions per `/dev` Phase 6. This is a PR-body note, not a gate —
do not stop.
</phase>
