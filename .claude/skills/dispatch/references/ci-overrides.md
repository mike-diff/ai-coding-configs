# /dispatch — CI overrides and hard-won constraints

`/dispatch` is `/dev` adapted for an unattended GitHub Actions run. This file records the
deltas from interactive `/dev` and the CI-specific constraints that were verified empirically
against `anthropics/claude-code-action@v1`.

## Deltas from interactive /dev

| /dev (interactive) | /dispatch (CI) |
|---|---|
...
- **Native nesting beyond one level is unreliable.** A sub-agent told to spawn its own
  ...
  Keep all delegation flat, one level. Do not design `/dispatch` around deep nesting.
...
4. **Imperative prompt.** A bare `/dispatch` makes the model DESCRIBE the skill instead of
