...
You (lead, delegate mode)
  ...
  └── Reviewer (read-only)
```
**When:** Cross-layer features spanning backend + frontend + tests. Auto-detected by `/dev`.
...
For `/discuss`, the COUNCIL pattern uses **progressive spawning** - teammates are
...
| Task | Pattern | Why |
|------|---------|-----|
| Rough idea, "what if..." | Progressive COUNCIL (`/discuss`) | Spawn/shutdown per phase during interview |
| Complex idea needing spec | Progressive COUNCIL (`/discuss` + DEEPEN) | Adds dependency researcher, produces phased spec |
| Single-layer feature | FLAT (`/dev`, auto-detected) | Simple coordination, lower cost |
| Fullstack feature (FE + BE) | HIERARCHICAL (`/dev`, auto-detected) | Separate file ownership per layer |
...
- `/dev` - implementation with auto-detected team shape (FLAT or HIERARCHICAL)
</team_patterns>
