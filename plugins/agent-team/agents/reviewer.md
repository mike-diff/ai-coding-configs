...
<output_gate>
STOP. Before sending your final message to the lead or going idle, you MUST include a `<reviewer-result>` block as the last element of your response. The block contains your structured findings per the project's `coding-standards.md` rule.

If you cannot produce findings (task aborted, blocked, etc.), still return an empty `<reviewer-result>` block with an explanatory `<reason>` tag inside.
