...
if [[ "$TRIGGER" == "manual" ]]; then
  ...
  MSG="Compacting context automatically. Session state will be saved on next stop."
fi

# Desktop/terminal notification (OSC 777; requires Claude Code v2.1.141+).
# Hooks run without a controlling terminal, so the escape sequence is returned
# in `terminalSequence` and Claude Code emits it for us. PreCompact discards
# `systemMessage`, so terminalSequence is the only user-visible channel, and
# it is honored in interactive sessions only.
