...
Trigger: a spec path with no single phase named — `/agent-team:dev @.context/specs/spec-X.md`,
`/agent-team:dev "all phases" @<spec>`. You orchestrate every phase end-to-end autonomously.
...
- Cap spend on headless runs: `claude -p "/agent-team:dev @<spec> --unattended" --max-budget-usd N`,
