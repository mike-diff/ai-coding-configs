# /agent-team:discuss — Full Phase Workflow

# /agent-team:discuss - Idea Exploration & Validated Planning
...
<constraints>
- Do NOT start implementing (that's what `/agent-team:dev` is for)
...
<context_management>
/agent-team:discuss uses the COUNCIL Agent Team pattern. Each research teammate has its own
...
<team_lifecycle>
The /agent-team:discuss command uses the COUNCIL team pattern with progressive spawning.
...
> **Spawn schemas:** The `team-orchestration` skill contains minimal reusable templates. The prompts below are the authoritative, context-specific versions for `/agent-team:discuss`. Follow these exactly — they include output budget instructions and mode-specific guidance not in the skill templates.
...
```
Create an agent team for this /agent-team:discuss session using the COUNCIL pattern.
...
```
"You are the codebase scout for a /agent-team:discuss session. The user is exploring this idea:
[PASTE IDEA]
...
```
"You are the codebase scout for a /agent-team:discuss REVISIT session. The user wants to
...
```
"You are the web researcher for a /agent-team:discuss session. The user is exploring this idea:
[PASTE IDEA]
...
<phase name="explore">
This is the core of `/agent-team:discuss` - an iterative, reactive conversation.
...
```
"You are the plan challenger for a /agent-team:discuss session. A plan has been drafted:
[PASTE DRAFT PLAN]
...
```
"You are the plan challenger for a /agent-team:discuss REVISIT session. A plan has been
...
recommended_next: /agent-team:spec "[spec-ready feature request]"
</adlc-handoff>
```
```
</output_format>
...
```
"You are the blind spot investigator for a /agent-team:discuss session. A plan has been
...
```
This is straightforward enough to implement directly. I still captured the ADLC handoff so /agent-team:dev or /agent-team:spec can preserve the validated assumptions.

Ready to build:
  /agent-team:dev [brief description based on plan]
```
...
research before handing off to /agent-team:dev? (Phase 5: DEEPEN)
...
- User says a `/agent-team:dev` command or "proceed": discussion is complete, skip to Report
...
<phase name="deepen">
Produce a phased implementation spec with dependency research. This replaces the need for a separate `/agent-team:spec` command.
...
Each phase is self-contained. An agent running `/agent-team:dev "Phase 1" @spec-file.md` should have 100% of the context needed without reading other phases.
...
Usage:
  /agent-team:dev @.context/specs/spec-[feature-name].md   (all phases, autonomous)
  /agent-team:dev "Implement Phase 0" @.context/specs/spec-[feature-name].md   (one phase)
```
...
<report>
When the user proceeds directly to `/agent-team:dev` without deepening:
...
`/agent-team:discuss` is the single entry point for all ideation and planning:

| Depth | When | Output | Next Step |
|-------|------|--------|-----------|
| **Light** | Small/clear feature | Validated plan (1-2 pages) | `/agent-team:dev [description]` |
| **Deep** | Complex/multi-phase feature | Phased spec with pinned deps (10+ pages) | `/agent-team:dev "Phase N" @.context/specs/spec-[name].md` |
