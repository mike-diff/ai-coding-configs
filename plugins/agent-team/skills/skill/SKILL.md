...
| `description` | Yes | 1–1024 chars. Must include **what** it does AND **when** to use it. Write in **third person** (no "I can…"/"You can…"). No XML tags. Include trigger keywords. |
...
**Naming:** prefer **gerund form** (`processing-pdfs`, `testing-code`); noun phrases (`pdf-processing`) and action-oriented (`process-pdfs`) are acceptable. Avoid vague (`helper`, `utils`), generic (`data`, `files`), and reserved words. See [naming conventions](references/agent-skills-spec.md).

---

## File Reference Rules

Use relative paths from the skill root. One level deep only. Forward slashes only.
...
❌ Never: `references\guide.md` — backslashes break on Unix; use `/`.
...
7. **Evaluate, don't assume** — Build evals from real gaps (run the task without the skill, record failures) before writing extensive content; aim for ≥3 scenarios and test across the models it will run under (Haiku/Sonnet/Opus).
</principles>
...
Apply the current Agent Skills best practices (see references/agent-skills-spec.md):
  - name: gerund form preferred; no XML tags, no reserved words (anthropic/claude)
  - description: third person (no "I/you can…"), what + when, no XML tags
  - file references one level deep, forward slashes only
  - reference files >100 lines get a ## Contents table of contents
  - scripts: solve-don't-punt, document constants, list deps, state execute-vs-read
  - build ≥3 evals from real gaps and test across target models (Haiku/Sonnet/Opus)
