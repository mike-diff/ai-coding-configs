# /agent-team:to-dos — Full Workflow

# /agent-team:to-dos - Technical Implementation Task Generator
...
| "This needs 20+ tasks" | Break into multiple `/agent-team:to-dos` invocations by feature area instead. |
...
<owner_assignment>
When tasks are created within a `/agent-team:dev` session (with an active team), assign owners
using `TaskUpdate`:
...
future `/agent-team:dev` session will claim tasks.
...
- **If implementation genuinely requires more than 15 tasks, STOP and suggest breaking into multiple `/agent-team:to-dos` invocations by feature area**
...
## Integration with /agent-team:dev

<dev_integration>
When `/agent-team:dev` needs to create its shared task list, it should follow this same task format.
...
If a `/agent-team:to-dos` session has already created tasks, `/agent-team:dev` can use `TaskGet` to read individual tasks and assign owners rather than recreating from scratch. Existing tasks are also visible in the Claude Code task UI.

### Handoff Pattern

```
User: /agent-team:to-dos Add authentication with JWT
  -> EnterPlanMode → explore → AskFollowupQuestion → plan → ExitPlanMode
  -> User approves → TaskCreate tasks with dependencies

User: /agent-team:dev Implement the authentication tasks
...
User: `/agent-team:to-dos Add dark mode toggle to settings page that persists preference to localStorage`
