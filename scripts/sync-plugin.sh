...
SKILL_NAMES='ask|code-review|dev|discuss|issue|loop-patterns|orient|primitives|skill|slop-check|spec|team-orchestration|testing-patterns|ticket|to-dos'
...
UNPREFIXED=$(perl -ne 'print "$ARGV:$.: $_" if /(?<![:\/\w-])\/(ask|code-review|dev|discuss|issue|loop-patterns|orient|primitives|skill|slop-check|spec|team-orchestration|testing-patterns|ticket|to-dos)\b(?!:)/' $(find "$DST/skills" -name "*.md") 2>/dev/null | grep -v "/agent-team:" || true)
