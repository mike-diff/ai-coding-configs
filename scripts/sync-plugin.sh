...
SKILL_NAMES='ask|dev|discuss|dispatch|goal-or-loop|issue|loop-patterns|orient|primitives|review-patterns|skill|slop-check|spec|team-orchestration|testing-patterns|ticket|to-dos'
...
UNPREFIXED=$(perl -ne 'print "$ARGV:$.: $_" if /(?<![:\/\w-])\/(ask|dev|discuss|dispatch|goal-or-loop|issue|loop-patterns|orient|primitives|review-patterns|skill|slop-check|spec|team-orchestration|testing-patterns|ticket|to-dos)\b(?!:)/' $(find "$DST/skills" -name "*.md") 2>/dev/null | grep -v "/agent-team:" || true)
